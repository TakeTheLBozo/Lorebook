...
function renderSection(sectionIndex) {
    ...
    leftPage.innerHTML = `<h2>${section.title}</h2>`;
    rightPage.innerHTML = "";

    const temp = document.createElement("div");
    temp.style.position = "absolute";
    temp.style.visibility = "hidden";
    temp.style.width = leftPage.clientWidth + "px";
    ...
    elements.forEach(el => {
        leftPage.appendChild(el.cloneNode(true));

        if (leftPage.scrollHeight > leftPage.clientHeight) {
            leftPage.removeChild(leftPage.lastChild);
            rightPage.appendChild(el.cloneNode(true));
        }
    });
